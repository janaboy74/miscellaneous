...
int truncate_file (wchar_t *filename, long truncLength) {
   ...
   /* Open a file */
    if( (fh = _wopen(filename, _O_RDWR))  != -1 )
    {
 ...
 */
int ReadMP3GainAPETag (wchar_t *filename, struct MP3GainTagInfo *info, struct FileTagsStruct *fileTags) {
    FILE *fi;
    long tag_offset, offs_bk;

    fi = _wfopen(filename, L"rb");
    if (fi == NULL)
		return 0;
 ...
 */
int WriteMP3GainAPETag (wchar_t *filename, struct MP3GainTagInfo *info, struct FileTagsStruct *fileTags, int saveTimeStamp) {
	FILE *outputFile;
	...
	outputFile = _wfopen(filename,L"r+b");
	if (outputFile == NULL) {
 ...
 */
int RemoveMP3GainAPETag (wchar_t *filename, int saveTimeStamp) {
