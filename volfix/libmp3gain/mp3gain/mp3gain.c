...
static
long getSizeOfFile(wchar_t *filename)
{
    long size = 0;
    FILE *file;
...
int deleteFile(wchar_t *filename)
{
    return _wremove(filename);
}

int moveFile(wchar_t *currentfilename, wchar_t *newfilename)
{
    return _wrename(currentfilename, newfilename);
}
...
void fileTime(wchar_t *filename, timeAction action)
{
...
static
#endif
int changeGain(wchar_t *filename AACGAIN_ARG(AACGainHandle aacH), int leftgainchange, int rightgainchange) {
  unsigned long ok;
  int mode;
  int crcflag;
  ...
  if (UsingTemp) {
	  fflush(stderr);
	  fflush(stdout);
 	  outlength = (long)wcslen(filename);
 	  outfilename = (wchar_t *)malloc(outlength+5);
      ...
      inf = _wfopen(filename,L"r+b");

	  if (inf != NULL) {
	    outf = _wfopen(outfilename, L"wb");
...
static
void WriteMP3GainTag(wchar_t *filename AACGAIN_ARG(AACGainHandle aacH), struct MP3GainTagInfo *info, struct FileTagsStruct *fileTags, int saveTimeStamp)
{
#ifdef AACGAIN
	if (aacH) {
...
void changeGainAndTag(wchar_t *filename AACGAIN_ARG(AACGainHandle aacH), int leftgainchange, int rightgainchange, struct MP3GainTagInfo *tag, struct FileTagsStruct *fileTag) {
...
static
int queryUserForClipping(wchar_t * argv_mainloop,int intGainChange)
{
	char ch;
...
static
void showVersion(wchar_t *progname) {
#ifdef AACGAIN
...
static
void errUsage(wchar_t *progname) {
...
#endif
	fwprintf(stderr,L"Usage: %s [options] <infile> [<infile 2> ...]\n",progname);
...
static
void fullUsage(wchar_t *progname) {
		showVersion(progname);
		fprintf(stderr,"copyright(c) 2001-2009 by Glen Sawyer\n");
#ifdef AACGAIN
	    fprintf(stderr,"AAC support copyright(c) 2004-2009 David Lasker, Altos Design, Inc.\n");
...
int main(int argc, char **argv) {
	...
	for (i = 1; i < argc; i++) {
#ifdef WIN32
		if ((argv[i][0] == '/')||((argv[i][0] == '-') && (wcslen(argv[i])==2))) { /* don't need to force single-character command parameters */
#else
		if (((argv[i][0] == '/')||(argv[i][0] == '-'))&&
			...
			switch(argv[i][1]) {
				...
				case 'D':
					if (argv[i][2] != '\0') {
						dBGainMod = _wtof(argv[i]+2);
					}
					else {
						if (i+1 < argc) {
							dBGainMod = _wtof(argv[i+1]);
							i++;
							fileStart++;
						}
						else {
				...
				case 'G':
					...
					if (argv[i][2] != '\0') {
						directGainVal = _wtoi(argv[i]+2);
					}
					else {
						if (i+1 < argc) {
							directGainVal = _wtoi(argv[i+1]);
							i++;
							fileStart++;
						}
						else {
				...
				case 'M':
					if (argv[i][2] != '\0') {
						mp3GainMod = _wtoi(argv[i]+2);
					}
					else {
						if (i+1 < argc) {
							mp3GainMod = _wtoi(argv[i+1]);
							i++;
							fileStart++;
						}
						else {
    ...
    for (mainloop = fileStart; mainloop < argc; mainloop++) {
      ...
      if (checkTagOnly) {
		  ...
		  if (tagInfo[mainloop].recalc > 0) {
              ...
              if (!aacH)
#endif
    			  inf = _wfopen(argv[mainloop],L"rb");
		  }
							...
							if (bitridx == 0) {
								...
								if (!QuietMode) {
									if ( !(++frame % 200)) {
                                        ...
                                        precentdisplay((int)(((double)(filepos-(inbuffer-(curframe+bytesinframe-buffer))) * 100.0) / gFilesize));
									}
								}
							}
						}
